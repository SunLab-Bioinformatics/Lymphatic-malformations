...
ggsave(file.path(outdir2,paste0(case,'UMAP_celltype.pdf')), last_plot(), height=2, width=4)

seurat_obj$group <- factor(seurat_obj$group, levels = c('Normal', 'LM', 'PE'))
p <- DimPlot(seurat_obj, reduction = "umap",split.by = 'group', 
             cols = type_colors, pt.size = 0.5, repel = T) + theme_bw() +
  labs( x= "UMAP 1",y= "UMAP 2",title = "") + 
  theme(panel.grid = element_blank(),aspect.ratio = 1)+NoLegend()
ggsave(file.path(outdir2,paste0(case,'split_group.pdf')), p, height=4, width=12)
...
s_term <- c(
  ...
  'Positive regulation of natural killer cell mediated cytotoxicity',
  'Positive regulation of natural killer cell activation')
p <- pheatmap(t(object[, colnames(object)%in%s_term]),cluster_cols = F,
...
ggsave(file.path(outdir2,'C5_group_s.pdf'), p, limitsize = FALSE, height=5, width=10)
...
tf.seleted <- c('IRF1(+)','ZNF44(+)','FOSL2(+)','FOXO1(+)','IKZF2(+)','NR3C1(+)','AHDC1(+)',
                'THAP1(+)','GATA3(+)','SPI1(+)','IKZF1(+)','EOMES(+)','IKZF3(+)','ELF4(+)','TBX21(+)')
pdf(file.path(outdir2, "TF_gene_group_N.pdf"), width = 5,height = 7)
pheatmap(
